...
> EOF
  ...
  }, [chatOpen, hasAwakened, dialogue.awakening]);
  ...
  return (
    <div className="min-h-screen pt-24 sm:pt-32 px-4 sm:px-6 relative">
      ...
      {chatOpen && (
        <div className={`fixed bottom-4 right-4 z-50 transition-all duration-300 ${
          chatMinimized ? 'w-16 h-16' : 'w-80 sm:w-100 h-[500px] md:h-[590px]'
        }`}>
          {chatMinimized ? (
            <button
              onClick={() => setChatMinimized(false)}
              className="w-full h-full bg-green-600 rounded-full flex items-center justify-center shadow-lg shadow-green-600/30 hover:bg-green-500 transition-colors animate-pulse"
            >
              <MessageSquare className="w-6 h-6 text-white" />
            </button>
          ) : (
